...
class Transformer(t.nn.Module):
    def __init__(self, args, matrix):
        ...
        self.encoder = Encoder(self.num_head, self.input_size, self.hidden_size, self.dropout, self.num_block, matrix,
                               self.encoder_max_lenth)
    ...
    def greedy_search(self, inputs, decode_lenth=None, if_sample=False):
        ...
        for i in range(step_cnt):
            output_token, output_probs = self.decoder(decoder_inputs=tokens, encoder_outputs=encoder_outputs,
    ...
    def beam_search(self, inputs, decode_lenth=None, if_sample=False):
        ...
        if decode_lenth is not None:
            ...
            step_cnt = self.decode_max_lenth
        for i in range(step_cnt):
            output_token, output_probs = self.decoder.beam_search(decoder_inputs=tokens,
...
class Decoder(t.nn.Module):
    ...
    def get_dot_attention_mask(self, inputs, encoder_mask):
        ...
        dot_attention_mask = (
                    1 - t.bmm(inputs_mask.unsqueeze(-1).float(), 1 - encoder_mask.data.unsqueeze(-2).float())).byte()
...
class DecoderBlock(t.nn.Module):
    ...
    def forward(self, inputs, encoder_outputs, attention_mask, dot_attention_mask, non_pad_mask):
        ...
        net = self.dot_multi_head_attention(net, encoder_outputs, encoder_outputs,
                                            dot_attention_mask)  # key,value = encoder_output
        net = net * non_pad_mask
...
class PositionEncoding(t.nn.Module):
    ...
    def init(self):
        position_enc = np.array(
            [[pos / np.power(10000, 2 * (j // 2) / self.embedding_dim) for j in range(self.embedding_dim)] if pos != 0
...
if __name__ == '__main__':
    ...
    with t.no_grad():
        for data in test_loader:
            ...
            token_id, prob_vector = mm.module.beam_search(context)
